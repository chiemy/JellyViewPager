 ...
 * 横向的ViewPager
 */
public class JellyHorizontalViewPager extends ViewPager {
    /**
     * View最小缩放比例
     */
    ...
    public JellyHorizontalViewPager(Context context) {
        super(context);
        init(context);
    }

    public JellyHorizontalViewPager(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    private void init(Context context) {
        super.setOnPageChangeListener(new MyPageChangeListener());
    ...
    protected void onLayout(boolean arg0, int arg1, int arg2, int arg3, int arg4) {
        ...
        mHeight = getHeight();
        mWidth = getWidth();
        OUT_DISTANCE_BOUDARY = MAX_SCALE * mWidth / 3;
    }
    ...
    private void animOutIfNeeded(float scrollDis, float velocityY) {
        ...
        if (velocityY > FLING_VELOCITY || (scrollDis > OUT_DISTANCE_BOUDARY)) {
            if (hasPre()) {
                ...
                || (scrollDis < -OUT_DISTANCE_BOUDARY)) { // 上一张
            if (hasNext()) {
                tranX = -mWidth;
                rotateSpring.setAtRest();
            } else {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if(Math.abs(distanceX) > mTouchSlop){
                    if(pageChangeListener != null){
                        pageChangeListener.onPageScrolled(currentItem, (int)Math.abs(distanceX)/getWidth(), (int)distanceX);
                        pageChangeListener.onPageScrollStateChanged(SCROLL_STATE_DRAGGING);
                    }
                    tranSpring.setEndValue(distanceX);
                    float degree = MAX_DEGREE * distanceX / mWidth;
                    if (downX < mWidth / 2) {
                        degree = -degree;
                    }
            ...
            case MotionEvent.ACTION_UP:
                if(Math.abs(distanceX) > mTouchSlop){
                    ...
                    animOutIfNeeded(currentX - downX, velocityX);
                    if (vTracker != null) {
                        vTracker.recycle();
                        vTracker = null;
                    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                break;
            case MotionEvent.ACTION_MOVE:
                float distance = Math.abs(currentX - downX);
